/**
 * 组合总和
 */
var testarr = [2, 3, 5];
var juagearr = function (bigarr, litarr) {
    var rejuage = true;
    for (var i = 0; i < bigarr.length; i++) {
        var nowju = true;
        if (litarr.length != bigarr[i].length) {
            rejuage == true;
            continue;
        }
        for (var j = 0; j < litarr.length; j++) {
            if (bigarr[i].indexOf(litarr[j]) === -1) {
                nowju = true;
                break;
            }
            else {
                nowju = false;
            }
        }
        rejuage = nowju;
        if (rejuage === false) {
            break;
        }
    }
    return rejuage;
};
var recurtionfun = function (testarr, target, nowremainder, rearr) {
    var nowarr = nowremainder.slice();
    var sunarr = 0;
    for (var k = 0; k < nowarr.length; k++) {
        sunarr += nowarr[k];
    }
    if (sunarr === target) {
        if (rearr.length == 0 || juagearr(rearr, nowarr)) {
            rearr.push(nowarr);
        }
        return;
    }
    else if (sunarr > target) {
        return;
    }
    sunarr = 0;
    for (var i = 0; i < testarr.length; i++) {
        nowarr.push(testarr[i]);
        for (var j = 0; j < nowarr.length; j++) {
            sunarr += nowarr[j];
        }
        if (sunarr === target) {
            if (rearr.length == 0 || juagearr(rearr, nowarr)) {
                rearr.push(nowarr);
            }
            return;
        }
        else if (sunarr > target) {
            return;
        }
        else {
            recurtionfun(testarr, target, nowarr, rearr);
        }
        sunarr = 0;
        nowarr.pop();
    }
};
var groupTotal = function (testarr, target) {
    var rearr = [];
    var nowremainder = [];
    for (var i = 0; i < testarr.length; i++) {
        recurtionfun(testarr, target, [testarr[i]], rearr);
    }
    return rearr;
};
console.log(groupTotal(testarr, 8));
